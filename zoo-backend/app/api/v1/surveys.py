...
def get_encuesta(encuesta_id: UUID, db: Session = Depends(get_db)):
...
def update_encuesta(encuesta_id: UUID, encuesta_in: EncuestaUpdate, db: Session = Depends(get_db)):
...
def delete_encuesta(encuesta_id: UUID, db: Session = Depends(get_db)):
...
def add_pregunta_a_encuesta(encuesta_id: UUID, pregunta_in: PreguntaEncuestaCreate, db: Session = Depends(get_db)):
...
def update_pregunta(pregunta_id: UUID, pregunta_in: PreguntaEncuestaUpdate, db: Session = Depends(get_db)):
...
def delete_pregunta(pregunta_id: UUID, db: Session = Depends(get_db)):
...
def add_opcion_a_pregunta(pregunta_id: UUID, opcion_in: OpcionEncuestaCreate, db: Session = Depends(get_db)):
    try:
...
def update_opcion(opcion_id: UUID, opcion_in: OpcionEncuestaUpdate, db: Session = Depends(get_db)):
...
def delete_opcion(opcion_id: UUID, db: Session = Depends(get_db)):
...
def get_participacion(participacion_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
...
def update_participacion(participacion_id: UUID, participacion_in: ParticipacionUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
    participacion = crud_survey.get_participacion(db, participacion_id)
    if not participacion or participacion.usuario_id != current_user.id:
        raise HTTPException(status_code=404, detail="Participacion no encontrada o no pertenece al usuario")
...
def delete_participacion(participacion_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
    participacion = crud_survey.get_participacion(db, participacion_id)
    if not participacion or participacion.usuario_id != current_user.id:
        raise HTTPException(status_code=404, detail="Participacion no encontrada o no pertenece al usuario")
...
def get_survey_stats(encuesta_id: UUID, db: Session = Depends(get_db)):
...
def get_respuesta(respuesta_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
...
def update_respuesta(respuesta_id: UUID, respuesta_in: RespuestaUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
    respuesta = crud_survey.get_respuesta(db, respuesta_id)
    if not respuesta or respuesta.participacion.usuario_id != current_user.id:
        raise HTTPException(status_code=404, detail="Respuesta no encontrada o no pertenece al usuario")
...
def delete_respuesta(respuesta_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
    respuesta = crud_survey.get_respuesta(db, respuesta_id)
    if not respuesta or respuesta.participacion.usuario_id != current_user.id:
        raise HTTPException(status_code=404, detail="Respuesta no encontrada o no pertenece al usuario")
